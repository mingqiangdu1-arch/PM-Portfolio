...
describe("realApi", () => {
  ...
  it("maps all frozen PRD and Design Review operations through the generated client", async () => {
    ...
    client.createProjectVersionPrd.mockResolvedValue(success({ prd, design_review: null }));
    client.getPrd.mockResolvedValue(success({ prd, design_review: review }));
    ...
    await expect(realApi.prds.get("prd-1")).resolves.toMatchObject({ prd: { currentVersionId: "prd-v1" }, review: { id: "review-1", prdVersionId: "prd-v1" } });
  ...
  it("loads completed project files for reload recovery", async () => {
    client.listProjectFiles.mockResolvedValue(success({
      items: [{
        file: { id: "file-1", project_id: "project-1", logical_name: "brief.txt", current_version_id: "version-1", status: "active", version: 2 },
        current_version: { id: "version-1", stored_file_id: "file-1", version_no: "V1", mime_type: "text/plain", extension: "txt", size_bytes: 5, checksum_sha256: "a".repeat(64), storage_status: "available", created_at: "2026-08-31T00:00:00Z" },
        relations: [],
      }],
    }));

    await expect(realApi.files.list("project-1")).resolves.toEqual([
      expect.objectContaining({ id: "version-1", name: "brief.txt", status: "uploaded", progress: 100 }),
    ]);
    expect(client.listProjectFiles).toHaveBeenCalledWith("project-1", expect.any(Object));
  });
