...
export function RequirementWorkspace({ projectVersionId, requirementId, api = frontendApi }: { projectVersionId: string; requirementId?: string; api?: FrontendApi }) {
  ...
  return <section aria-labelledby="requirement-title" className="space-y-token-lg">
    ...
    {requirement ? <>
      ...
      {isRequirementComplete ? <StatusPanel tone="success" title="Requirement 已完成"><p>Baseline 已确认，版本 {requirement.effectiveVersion?.versionNo ?? "—"} 已设为当前 Baseline。</p><p className="mt-token-xs text-sm text-secondary">{isAiSourcedEffectiveBaseline ? "来源：AI 候选采用。" : "来源：人工 Baseline 确认。"} 当前 Requirement / Baseline 阶段已完成；PRD 为后续阶段，当前在线 Demo 未启用。</p></StatusPanel> : null}
