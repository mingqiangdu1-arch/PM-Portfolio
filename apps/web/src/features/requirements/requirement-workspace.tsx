...
function pendingDeepContinuationRound(version: RequirementView["currentVersion"] | undefined): ClarificationRoundView | null {
  const clarification = version?.content.clarification;
  if (!clarification || clarification.mode !== "deep" || clarification.finishReason !== null || clarification.continueDeepConfirmed || clarification.rounds.length !== 3) return null;
  const completedRoundNumbers = new Set(clarification.rounds.map((round) => round.roundNo));
  if (completedRoundNumbers.size !== 3 || ![1, 2, 3].every((roundNo) => completedRoundNumbers.has(roundNo))) return null;
  const roundThree = clarification.rounds.find((round) => round.roundNo === 3);
  if (!roundThree || roundThree.questions.length === 0 || roundThree.questions.length !== roundThree.answers.length) return null;
  const questionIds = new Set(roundThree.questions.map((question) => question.questionId));
  const answerIds = new Set(roundThree.answers.map((answer) => answer.questionId));
  if (questionIds.size !== roundThree.questions.length || answerIds.size !== roundThree.answers.length || ![...questionIds].every((questionId) => answerIds.has(questionId))) return null;
  return roundThree;
}
...
export function RequirementWorkspace({ projectVersionId, projectId, requirementId, api = frontendApi }: { projectVersionId: string; projectId?: string; requirementId?: string; api?: FrontendApi }) {
  ...
  const deepContinuationRound = pendingDeepContinuationRound(currentVersion);
  ...
  const loadRequirement = useCallback(async (selectedRequirementId?: string) => {
    setBusy(true); setError(""); setNotice(""); setRecoveryError(""); setResultRecoveryError(""); setRequirement(null); setRequirementOptions([]); setTask(null); setResult(null); setAnswers({}); setDeepContinueConfirmed(false); setCandidateBaseline(null); setResultRecoveryState("idle"); setRecoveryState("loading");
    try {
  ...
  return <section aria-labelledby="requirement-title" className="space-y-token-lg">
    ...
    {requirement ? <>
      ...
      {questions.length ? <article className="panel space-y-token-md"><h2 className="font-semibold">3. 第 {result?.roundNo ?? 1} 轮问题（{questions.length}/3）</h2>{questions.map((question) => <div key={question.questionId}><label className="field-label" htmlFor={question.questionId}>{question.questionText}</label><textarea id={question.questionId} className="textarea-field min-h-24" value={answers[question.questionId] ?? ""} onChange={(event) => setAnswers((old) => ({ ...old, [question.questionId]: event.target.value }))} /></div>)}<div className="flex flex-wrap gap-token-sm"><Button variant="secondary" onClick={() => submitAnswers(false)} loading={busy}>保存回答</Button><Button onClick={() => submitAnswers(true)} loading={busy}>结束并审核需求基线</Button></div></article> : null}
      {deepContinuationRound ? <article className="panel space-y-token-md"><h2 className="font-semibold">确认继续深度澄清</h2><p className="text-sm text-secondary">已完成深度澄清前三轮。此确认只授权进入第 4 轮，不会确认需求基线，也不会自动采用 AI 候选。</p><label className="flex items-center gap-token-sm text-sm"><input type="checkbox" checked={deepContinueConfirmed} onChange={(event) => setDeepContinueConfirmed(event.target.checked)} />我确认继续深度澄清第 4 轮</label><Button onClick={confirmDeepContinuation} loading={busy} disabled={!deepContinueConfirmed}>确认继续深度澄清</Button></article> : null}
